...
connection = create_db_connection("localhost", "root", "abcd1", "stockapp")
username = "User"
userid = 1
...
def main(page: ft.Page):
    ...
    def req_register(e, email, password, usernameinput):
        global username
        username = usernameinput
        query = "INSERT INTO users (email, password, username) VALUES ('" + email + "', '"\
                + password + "', '" + usernameinput + "');"
        regcheck = execute_query(connection, query)
        if regcheck:
            snack.content.value = "Registered Successfully!."
            snack.open = True
            page.update()
        else:
    ...
    def req_login(e, email, password):
        ...
        query2 = "SELECT user_id FROM users WHERE email ='" + email + "' and password = '" + password + "'"
        result2 = read_query(connection, query2)
        if result:
            global username
            temp_string = result[-1]
            username = temp_string[0].strip('[]').strip('""')
            global userid
            temp_string2 = result2[-1]
            userid = temp_string2[0]
            print(userid)
            page.go("/dash")
    ...
    def stockupdater(e):
        ...
        stock_query = ("INSERT INTO stocks (stocksticker, CurrentPrice) VALUES ('" + stockname + "'," +
                       str(int(cpricetext)) + ") ON DUPLICATE KEY UPDATE CurrentPrice = " + str(int(cpricetext)) + ";")
        ...
        stockanalysis_query = ("INSERT INTO stockanalysis (StockTicker, PredictedPrice) VALUES ('" + stockname + "'," +
                       str(int(ppricetext)) + ");")
        execute_query(connection, stockanalysis_query)

        # Payment Button interaction
        def purchase_button(e):
            purchase_query = "INSERT INTO transaction (price, user_ID, stockticker) VALUES (" + str(int(cpricetext)) + "," + str(userid) + ",'"+ stockname+ "');"
            execute_query(connection, purchase_query)
            page.update()
        ...
        paymentcolumn = ft.Column([
            ...
            cvv,
            card_name,
            ft.ElevatedButton("Purchase Stock", on_click=purchase_button),
            payment, ])
        ...
        ft.Container(
            content=ft.Row([
                ft.IconButton(icon=ft.icons.ARROW_BACK_SHARP,
                              icon_color=ft.colors.WHITE, icon_size=32,
                              on_click=lambda _: page.go("/dash")),
                ft.Text(" ", size=30, color=ft.colors.WHITE, font_family="SF Pro"),
    ...
    def transactionupdater(e):
        page.update()

        query = "SELECT stockticker, SUM(quantity) AS total_quantity, AVG(price) AS average_price FROM transaction WHERE user_id = " + str(userid) + " GROUP BY stockticker;"
        result = read_query(connection, query)
        tstock, tquantity, tprice = zip(*result)
        transaction_table = ft.DataTable(
            width=1800,
            bgcolor=ft.colors.BLACK12,
            border=ft.border.all(2),
            border_radius=10,
            vertical_lines=ft.border.BorderSide(3),
            ...
            columns=[
                ...
                ft.DataColumn(ft.Text("Current Price")),
                ft.DataColumn(ft.Text("Quantity"), numeric=True),
            ],
            rows=[
                ft.DataRow(
                    cells=[
                        ft.DataCell(ft.Text(tstock[0])),
                        ft.DataCell(ft.Text(tprice[0])),
                        ft.DataCell(ft.Text(tquantity[0])),
                    ],
                ),
                ft.DataRow(
                    cells=[
                        ft.DataCell(ft.Text(tstock[1])),
                        ft.DataCell(ft.Text(tprice[1])),
                        ft.DataCell(ft.Text(tquantity[1])),
                    ],
                ),
                ft.DataRow(
                    cells=[
        ...
        transaction_column = ft.Column([usertrccolumn, transaction_table], alignment=ft.MainAxisAlignment.SPACE_EVENLY)
        page.views.append(
            View(
                controls=[ft.Container(content=transaction_column,
                                       border_radius=10, width=1990,
                                       bgcolor=ft.colors.BLACK,
                                       padding=ft.padding.symmetric(vertical=10, horizontal=10), )],
            )
                                ...
                                content=Container(
                                    ...
                                    content=Column(
                                        ...
                                        controls=[
                                            ...
                                            Text(
                                                "Enter an E-mail address, set username and password, and get started!",
                                                size=14,
                                                weight="w700",
                                                text_align="center",
                                                color="#64748b",
